...
///
/// **Spike 014 (#682), branch-only.** This is the streaming primitive
/// spec 013/Req-7 needs: `execute_with_db` and friends materialize every
/// row into a `Vec` before the caller sees the first one, which a
/// `Statement::next_row` facade cannot be built on top of. Nothing here
/// changes what a program computes — [`run`] below is now a thin wrapper
/// that collects `next_row` into the same `Vec` it always returned, so
/// the batch path and the streaming path are literally the same loop.
/// That is what makes the change additive: a behaviour difference
/// between them would be a bug in one, not a divergence callers have to
/// reason about.
///
