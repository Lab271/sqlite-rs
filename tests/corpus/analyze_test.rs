...
/// check for opcodes (e.g. `OpenEphemeral`/`AutoIndexSeek`, #545) that
/// `EXPLAIN QUERY PLAN`'s human-readable summary doesn't surface.
...
/// #545: once `ANALYZE` shows a join level's table has enough rows and
/// no rowid/unique-index seek is structurally available, the compiled
/// program prefaces that level's nested-loop scan with a transient
/// automatic index (`OpenEphemeral` + `AutoIndexSeek`, see
/// `join_access::choose_auto_index_probe`) -- and the join still
/// returns the same rows as the oracle.
#[test]
